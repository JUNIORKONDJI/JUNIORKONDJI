# 👋 Hi, I'm Junior Kondji
  
Currently studying **Master’s in Computing Systems & Networks (LM-32)** at Università degli Studi di Firenze (UNIFI)  
Bachelor’s in Computer Engineering (L-8), Università degli Studi di Firenze  

### My Projects
You can find all my projects and contributions on my [GitHub profile](https://github.com/JUNIORKONDJI).  
Highlights include:  
- **Portfolio Website** – HTML/CSS/JS, responsive design  
- **Java Project** – Data management system with SQL backend  
- **Python Security Tool** – Network analysis and security utilities  

### How to reach me
[Email](mailto:kondjijunior@gmail.com) | [GitHub](https://github.com/JUNIORKONDJI) | [LinkedIn](https://www.linkedin.com/in/JUNIORKONDJI)
